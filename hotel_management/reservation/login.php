...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($loged) {
        if($loged['isLogged']) {
            ...
            $errors['pwd'] = 'Đăng nhập không hợp lệ. Kiểm tra email/mật khẩu';
        }  
    } else {
        $errors['pwd'] = 'Đăng nhập không hợp lệ. Kiểm tra email/mật khẩu';
    }  
  ...
  <body>
    <div class="form-container-det" id="login">
        <form class="add-form" method="POST" enctype="multipart/form-data" style="max-width: 468px; margin: 0 auto;">
                ...
                <small>Không có tài khoản? <a href="./register.php">Đăng ký</a></small> |
                <small><a href="./reset/request.php">Quên mật khẩu</a></small>
            </form>
        </div>
