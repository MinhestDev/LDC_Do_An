...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if(!preg_match("/^[a-z ,.'-]+$/i", $fname)){
      $errors['fname'] = "Mục nhập tên không hợp lệ";
   }
   if(!preg_match("/^[a-z ,.'-]+$/i", $lname)){
      $errors['lname'] = "Mục nhập tên không hợp lệ";
   }
   if(!preg_match("/^\d+$/", $dnum)){
      $errors['dnum'] = "Mục nhập Nhận dạng không hợp lệ";
   }
   if(!preg_match("/^07\d{8}$/", $mobile)){
      $errors['mobile'] = "Mục nhập số điện thoại không hợp lệ";
   }
   if(!preg_match("/^[a-zA-Z0-9._%+-]+@(gmail|hotmail|yahoo)\.com$/", $email)){
      $errors['email'] = "Mục nhập Email không hợp lệ";
   }
   if(!preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/", $pwd)) {
         $errors['pwd'] = "Mật khẩu phải chứa 8 hoặc ký tự, chữ in hoa và ký tự đặc biệt";
   }
   if($cpwd !== $pwd) {
         $errors['pwd1'] = "Mật khẩu không phù hợp";
   }
  ...
  <body>
    <div class="form-container-det">
        <form class="add-form" method="POST" enctype="multipart/form-data" style="max-width: 648px; margin: 0 auto;"> 
            ...
            <div class="form-group">
                <label for="exampleInputEmail1">Số CMND/Hộ chiếu</label>
